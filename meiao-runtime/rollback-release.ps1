...
function Copy-BackupEntries {
  param(
    [string]$BackupRuntimeRoot,
    [string]$RuntimeRoot
  )
  foreach ($child in Get-ChildItem -LiteralPath $BackupRuntimeRoot -Force) {
    if ($child.Name -ieq "integrations" -and $child.PSIsContainer) {
...
if (-not (Test-Path -LiteralPath $TargetRoot)) {
  throw "Rollback target not found: $TargetRoot"
}
...
if (-not (Test-Path -LiteralPath $backupRuntimeRoot)) {
  throw "Rollback backup payload not found: $backupRuntimeRoot"
}

Remove-ProgramEntries $targetFull
Copy-BackupEntries $backupRuntimeRoot $targetFull

Write-Host "Release rolled back:"
