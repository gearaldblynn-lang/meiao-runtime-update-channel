...
# 剪映草稿保存路径（下载剪映草稿保存位置）。
# 5.9 的用户草稿通常在 D:\JianyingPro Drafts；启动脚本会优先注入该路径。
DEFAULT_JIANYING_59_DRAFT_PATH = r"D:\JianyingPro Drafts"
DRAFT_SAVE_PATH = os.getenv("DRAFT_SAVE_PATH") or (
    DEFAULT_JIANYING_59_DRAFT_PATH
    if os.path.exists(DEFAULT_JIANYING_59_DRAFT_PATH)
    else os.path.join(os.path.expanduser("~"), "AppData", "Local", "JianyingPro", "User Data", "Projects", "com.lveditor.draft")
)

# 草稿模板固定使用 5.9 可打开的低版本结构；default2 的 110.0.0 容易触发低版本升级提示。
JY_DRAFT_TEMPLATE = os.getenv("JY_DRAFT_TEMPLATE", "default")
...
# Max concurrent draft file downloads. Jianying export itself remains serialized by export_video_lock.
DRAFT_DOWNLOAD_MAX_WORKERS = max(1, min(8, int(os.getenv("DRAFT_DOWNLOAD_MAX_WORKERS", "4"))))
