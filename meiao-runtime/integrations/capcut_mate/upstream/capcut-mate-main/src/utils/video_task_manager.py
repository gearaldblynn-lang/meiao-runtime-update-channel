...
class VideoGenTaskManager:
    ...
    def _check_draft_duration(self, task: VideoGenTask) -> bool:
        ...
        try:
            ...
            if not os.path.exists(draft_content_path) and not os.path.exists(draft_info_path):
                logger.error(
                    "draft_content/draft_info not found: path=%s draft_id=%s",
                    draft_content_path,
                    task.draft_id,
                )
                return False

            duration = self._read_draft_duration_us(draft_content_path, draft_info_path)
            ...
            logger.error(
                "failed to parse draft duration JSON: %s draft_id=%s",
                e,
                task.draft_id,
            )
            return False
    ...
    def _read_draft_duration_us(self, *paths: str) -> int:
        """读取草稿时长。5.9 打开后可能会加密 draft_content，保留 draft_info 明文作为兜底。"""
        last_error = None
        for path in paths:
            ...
            try:
                ...
                last_error = exc
                logger.warning("draft duration file is not plain JSON, skip: %s", path)
            except Exception as exc:
                last_error = exc
                logger.warning("failed to read draft duration file %s: %s", path, exc)
        if last_error:
            raise last_error
        return 0
    ...
    def _phase_cos_upload_finalize(self, task: VideoGenTask) -> Tuple[str, str]:
        ...
        try:
            ...
            if upload_failed:
                local_url = self._build_local_output_url(task.outfile)
                self._cleanup_files(task.outfile, task.draft_id, keep_video=True)
                logger.warning(
                    "Video export succeeded but object storage upload failed; "
                    "kept local output: %s",
                    local_url,
                )
                return local_url, ""
            self._cleanup_files(task.outfile, task.draft_id, keep_video=False)
    ...
    def _build_local_output_url(self, outfile: str) -> str:
        """
        将本地导出文件转换成 capcut-mate 静态服务可访问地址。
        """
        filename = os.path.basename(outfile)
        return f"http://127.0.0.1:30000/output/draft/{quote(filename)}"
    ...
    def _cleanup_files(self, outfile: str, draft_id: str, *, keep_video: bool = False) -> None:
        """
        清理临时文件
        ...
        try:
            # 清理本地视频文件
            if os.path.exists(outfile) and not keep_video:
                os.remove(outfile)
                logger.info(f"Cleaned up local video file: {outfile}")
            elif os.path.exists(outfile) and keep_video:
                logger.info(f"Kept local video file: {outfile}")
