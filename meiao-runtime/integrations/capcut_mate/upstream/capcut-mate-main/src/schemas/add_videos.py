...
class AddVideosResponse(BaseModel):
    ...
    segment_infos: List[SegmentInfo] = Field(default=[], description="Segment info list")
    total_duration: int = Field(default=0, description="视频轨道总时长（微秒）")
    source_duration_total: int = Field(default=0, description="真实视频素材源时长总和（微秒）")
