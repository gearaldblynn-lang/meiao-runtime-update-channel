...
from src.schemas.recognize_subtitles import RecognizeSubtitlesRequest, RecognizeSubtitlesResponse
...
async def add_videos(avr: AddVideosRequest) -> AddVideosResponse:
    ...
    """
    # 调用 service 层处理业务逻辑（异步版本，带锁保护）
    draft_url, track_id, video_ids, segment_ids, total_duration, source_duration_total, segment_infos = await service.add_videos_async(
...
@router.post(path="/recognize_subtitles", response_model=RecognizeSubtitlesResponse)
def recognize_subtitles(rsr: RecognizeSubtitlesRequest) -> RecognizeSubtitlesResponse:
    """
    打开剪映草稿，优先通过音频片段右键菜单触发“识别字幕/歌词”。
    """
    result = service.recognize_subtitles(
        draft_url=rsr.draft_url,
        draft_name=rsr.draft_name,
        timeout=rsr.timeout,
    )
    return RecognizeSubtitlesResponse(**result)
