...
def process_audio_duration(audio: dict, actual_duration: int):
    ...
    if audio.get("speed"):
        ...
        logger.info(f"Using detected audio duration for speed mode: {actual_duration} microseconds")
    elif audio.get('duration') is None:
        audio['duration'] = actual_duration
...
def calculate_adjusted_time_range(audio: dict, actual_duration: int):
    ...
    if audio.get("speed"):
        ...
        logger.info(
            f"Audio explicit speed mode: start={start_time}, end={end_time}, "
            f"duration={segment_duration}, source_duration={actual_duration}, speed={audio.get('speed')}"
        )
...
def create_audio_segment(audio_path: str, start_time: int, segment_duration: int, audio: dict):
    ...
    if speed:
        source_duration = max(1, int(audio.get("duration") or 0))
        playback_speed = source_duration / max(1, segment_duration)
